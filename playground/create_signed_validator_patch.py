...
"""
Update trust for a validator
PATCH /validator/<node_identifier>
"""
...
def run(send_to_node=False):
    ...
    target_nid = PV_NID_ACCOUNT_NUMBER
    ...
    write_json(
        os.path.join(SIGNED_REQUESTS_DIR, 'validator-patch-request.json'),
        signed_request
    )
...
def send_request_to_node(signed_request, target_nid):
    ...
    url = f'{node_address}/validators/{target_nid}'
    ...
    write_json(
        os.path.join(SIGNED_REQUESTS_DIR, 'validator-patch-response.json'),
        results
    )
